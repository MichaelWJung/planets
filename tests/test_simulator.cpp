...
TEST(SimulatorTest, TwoBodiesAttractEachOther) {
  // Body 0 (mass m) at (-d, 0, 0), body 1 (mass 2m) at (+d, 0, 0), both at
  ...
  Body body0{
      ...
      .position = position_t{cartesian_vector<double>{
                                 -d.numerical_value_in(metre), 0.0, 0.0} *
                             isq::displacement[metre], solar_system_center_of_mass},
      .velocity = velocity_t{},
  };
  Body body1{
      .mass = 2.0 * m,
  ...
  // body 1 (at +x). A sign check on one component is unavoidable here; mp-units
  // has no ordering on vector quantities.
...
TEST(SimulatorTest, SingleBodyWithVelocity) {
  ...
  auto vel = velocity_t{cartesian_vector<double>{1.0, 0.0, 0.0},
                        isq::velocity[metre / second]};
  Body body{
      .mass = 1.0 * kilogram,
      .position = position_t{},
      .velocity = vel,
  ...
  auto expected_pos = position_t{cartesian_vector<double>{1.0, 0.0, 0.0} *
                                 isq::displacement[metre], solar_system_center_of_mass};
  const auto &state = sim.getState();
